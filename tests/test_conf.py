...
from devxhub import const


@pytest.fixture
def load_source(mocker):
    return mocker.patch('devxhub.conf.load_source')
...
class TestSettingsFromEnv(object):
    def test_from_env(self, os_environ, settings):
        os_environ.update({'DEVXHUB_RULES': 'bash:lisp',
                           'DEVXHUB_EXCLUDE_RULES': 'git:vim',
                           'DEVXHUB_WAIT_COMMAND': '55',
                           'DEVXHUB_REQUIRE_CONFIRMATION': 'true',
                           'DEVXHUB_NO_COLORS': 'false',
                           'DEVXHUB_PRIORITY': 'bash=10:lisp=wrong:vim=15',
                           'DEVXHUB_WAIT_SLOW_COMMAND': '999',
                           'DEVXHUB_SLOW_COMMANDS': 'lein:react-native:./gradlew',
                           'DEVXHUB_NUM_CLOSE_MATCHES': '359',
                           'DEVXHUB_EXCLUDED_SEARCH_PATH_PREFIXES': '/media/:/mnt/'})
        settings.init()
    ...
    def test_from_env_with_DEFAULT(self, os_environ, settings):
        os_environ.update({'devxhub_RULES': 'DEFAULT_RULES:bash:lisp'})
        settings.init()
...
@pytest.mark.parametrize('legacy_dir_exists, xdg_config_home, result', [
    (False, '~/.config', '~/.config/devxhub'),
    (False, '/user/test/config/', '/user/test/config/devxhub'),
    (True, '~/.config', '~/.devxhub'),
    (True, '/user/test/config/', '~/.devxhub')])
def test_get_user_dir_path(mocker, os_environ, settings, legacy_dir_exists,
                           xdg_config_home, result):
    mocker.patch('devxhub.conf.Path.is_dir',
