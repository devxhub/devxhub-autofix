...
import os
import pytest
from devxhub.shells import Bash
...
class TestBash(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('devxhub.shells.bash.Popen')
        return mock

    @pytest.fixture(autouse=True)
    def shell_aliases(self):
        os.environ['TF_SHELL_ALIASES'] = (
            'alias dxh=\'eval $(devxhub $(fc -ln -1))\'\n'
            'alias l=\'ls -CF\'\n'
            'alias la=\'ls -A\'\n'
            'alias ll=\'ls -alF\'')
    ...
    def test_get_aliases(self, shell):
        assert shell.get_aliases() == {'dxh': 'eval $(devxhub $(fc -ln -1))',
                                       'l': 'ls -CF',
                                       'la': 'ls -A',
                                       'll': 'ls -alF'}

    def test_app_alias(self, shell):
        assert 'dxh () {' in shell.app_alias('dxh')
        assert 'DXH () {' in shell.app_alias('DXH')
        assert 'devxhub' in shell.app_alias('dxh')
        assert 'PYTHONIOENCODING' in shell.app_alias('dxh')

    def test_app_alias_variables_correctly_set(self, shell):
        alias = shell.app_alias('dxh')
        assert "dxh () {" in alias
        assert 'TF_SHELL=bash' in alias
        assert "TF_ALIAS=dxh" in alias
