...
import pytest
from devxhub.shells.tcsh import Tcsh
...
class TestTcsh(object):
    ...
    def Popen(self, mocker):
        mock = mocker.patch('devxhub.shells.tcsh.Popen')
        mock.return_value.stdout.read.return_value = (
            b'dxh\teval $(devxhub $(fc -ln -1))\n'
            b'l\tls -CF\n'
            b'la\tls -A\n'
            b'll\tls -alF')
        return mock
    ...
    def test_app_alias(self, shell):
        assert 'setenv TF_SHELL tcsh' in shell.app_alias('dxh')
        assert 'alias dxh' in shell.app_alias('dxh')
        assert 'alias DXH' in shell.app_alias('DXH')
